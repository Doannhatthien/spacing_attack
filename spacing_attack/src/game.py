...
from .settings import WIDTH, HEIGHT, FPS, WHITE, WORDS, SPAWN_DELAYMS, SHIP_Y, SHIP_WIDTH
...
# Lớp quản lý toàn bộ logic game
class Game:
    """
    ...
    def __init__(self, music_file=None, video_background=None, challenge_speed=None):
        ...
        # thay đổi số mạng tăng hoặc giảm cho phi thuyền
        # thêm font chữ mới cho game
        ...
        # chỉnh lần 1 trong game.py
        # Đổi thành ảnh phi thuyền khác
        self.ship_skin = "spaceship.png"  # Mặc định
        # Có thể đổi thành: "ship2.png", "ship3.png"
        # ============================================================
        # VỊ TRÍ PHI THUYỀN CỐ ĐỊNH (Horizontal Position)
        # ============================================================
        # Phi thuyền sẽ đứng yên ở vị trí này (không di chuyển)
        # Bạn có thể thay đổi giá trị self.ship_x để đặt phi thuyền ở vị trí khác:
        # - Bên trái: self.ship_x = 100
        # - Giữa màn hình: self.ship_x = WIDTH // 2  (mặc định)
        # - Bên phải: self.ship_x = WIDTH - 100
        # - Hoặc bất kỳ giá trị nào từ 50 đến WIDTH-50
        
        try:
            from .settings import SHIP_X
            self.ship_x = SHIP_X  # Lấy từ settings.py nếu có
        except Exception:
            self.ship_x = WIDTH // 2  # Mặc định: giữa màn hình
        ...
        # ============================================================
        try:
            # Load explosion image without forcing a fixed resize here.
            # Explosion.draw will scale the image based on animation scale.
            self.explosion_img = load_image("explosion.png")
        except:
        ...
        # ============================================================
        # thay đổi âm lượng tất cả các hiệu ứng âm thanh
        try:
        ...
        try:
            self.warning_sound = load_sound("canhbao.wav")
            if self.warning_sound:
                self.warning_sound.set_volume(0.7)
        except Exception as e:
            print(f"[WARN] Không thể nạp âm thanh canhbao.wav: {e}")
        ...
        #Load nhạc nền cho game trong chế độ chailenge
        ...
        # Warning system
        self.is_warning_active = False  # Theo dõi trạng thái cảnh báo
        self.warning_channel = None  # Kênh riêng cho âm thanh cảnh báo
    ...
    def _update_ship_aim(self):
        ...
        if self.locked and self.locked in self.enemies:
            ...
            ty = self.locked.y
            # Aim relative to current ship_x (not always center)
            self.angle = math.atan2(ty - SHIP_Y, tx - (self.ship_x))
        else:
            self.angle = 0.0
    ...
    def _draw_hearts(self, x, y):
        ...
        for i in range(self.max_lives):
            ...
            # Chọn màu theo quả tim còn lại (có thể tùy chỉnh)
            if i < self.lives:
                if self.lives == 1:
    ...
    def hit_ship(self, enemy):
        ...
        self.lives -= 1
        # Tạo vụ nổ tại vị trí hiện tại của tàu
        self.explosions.append(Explosion(self.ship_x, SHIP_Y, lifetime_frames=40))
    ...
    def destroy_enemy(self, enemy):
        ...
        """
        # thay đổi điểm số nhận được khi tiêu diệt enemy
        self.score += len(enemy.origin_word) * 10
        self.kills += 1
        #hiệu ứng nổ chậm được gọi trong game.py
        self.explosions.append(Explosion(enemy.x, enemy.y, lifetime_frames=30))
        # Tùy chỉnh hiệu ứng rung màn hình cho game
    ...
    def handle_typed_char(self, ch):
        ...
        if not self.locked:
            ...
            if candidates:
                ...
                self.typed_word = ch
                # Tạo bullet từ vị trí phi thuyền hiện tại
                self.bullets.append(Bullet(self.locked, ch, self.font, self.locked.progress, 
                                          ship_x=self.ship_x, ship_y=SHIP_Y))
    ...
    def draw(self):
        ...
        # vị trí xoay của phi thuyền
        if draw_ship_flag:
            if self.locked and self.locked in self.enemies:
                draw_rotated_ship(self.win, self.angle, shake_offset, center_x=self.ship_x, ship_skin=self.ship_skin)
            else:
        # Xóa phi thuyền
                draw_ship(self.win, shake_offset, center_x=self.ship_x, ship_skin=self.ship_skin)
        ...
        if dangerous_enemies:
            ...
            # Phát âm thanh cảnh báo (chỉ khi chưa đang phát)
            if not self.is_warning_active and self.warning_sound:
                # Tạo hoặc sử dụng kênh riêng cho warning
                if self.warning_channel is None:
                    self.warning_channel = pygame.mixer.Channel(6)  # Kênh 6 cho warning
                
                # Phát âm thanh cảnh báo lặp lại
                self.warning_channel.play(self.warning_sound, loops=-1)  # loops=-1 = lặp vô hạn
                self.is_warning_active = True
                print("⚠️ WARNING: Enemy gần phi thuyền! Âm thanh cảnh báo đã phát!")
        else:
            # Dừng âm thanh cảnh báo khi không còn nguy hiểm
            if self.is_warning_active and self.warning_channel:
                self.warning_channel.stop()
                self.is_warning_active = False
                print("✅ An toàn: Âm thanh cảnh báo đã tắt")
